...
const matchInputValidation = (
  ...
  state[stateKey].validation = {
    ...
    ...(keyPath === "match"
      ? {
          match: matchKey,
          trackingMatch: setTrackingMatching(state[stateKey], [
...
const matchRules = (state: ObjState) => {
  ...
  for (const stateKey in state) {
    if (stateKey in state) {
      /*
       * We check if user want custom errorMessage and make validate unable to generate error message
       * */
      const errorMessage = state[stateKey].errorMessage;
      mappedState[stateKey]._ = !(
        errorMessage !== "undefined" &&
        errorMessage !== "" &&
        errorMessage !== null
      );
      // we save the error message
      mappedState[stateKey].___ = errorMessage;
